...
write_csv(results,
          paste0('//smb-main.ad.bcm.edu/genepi/TINMAN/Metabolomics/R_outputs/mwas_results_unscaled_', format(Sys.Date(), '%Y%m%d'), '.csv'))

# Volcano plot ------------------------------------------------------------

plot.data <- results %>% 
  select(CHEMICAL_NAME, delta.or, baseline.or) %>% 
  pivot_longer(cols = !CHEMICAL_NAME,
               names_to = c('measurement', 'p.value'))

plot <- ggplot(data = results, aes(x = log2(delta.or), y = -log10(p.value.delta))) + 
  
  geom_point(aes(color = SUPER_PATHWAY)) + 
  ...
  geom_text_repel(aes(label = ifelse(p.value.delta < 0.05, CHEMICAL_NAME, '')),
